...
class GeminiConfig {
  ...
  static Future<void> initialize() async {
    try {
      ...
      debugPrint('Erru inisializasaun Gemini: $e');
      rethrow;
    }
  }
  ...
  static Future<String> generateResponse(String prompt) async {
    try {
      ...
      if (response.text == null || response.text!.isEmpty) {
        throw Exception('Resposta mamuk husi Gemini');
      }

      return response.text!;
    } catch (e) {
      debugPrint('Erru resposta Gemini nian: $e');
      return 'Deskulpa, mosu erru ida iha prosesamentu mensajen nian. Favor ida koko fali depois. (Error: $e)';
    }
  }
...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  Future<void> _clearChatHistory() async {
    try {
      ...
      debugPrint('Erru hamoos istória konversa nian: $e');
    }
  }
  ...
  Future<void> _initializeChat() async {
    ...
    await _firestore.collection('ai_chat_messages').add({
      'isBot': true,
      'message':
          'Óla! Ha u hanesan **Asistente Saúde** ida ne ebé prontu atu ajuda ita-boot!',
      'showFeatures': true,
  ...
  Future<void> _handleSendMessage() async {
    if (!_isInitialized) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Favor hein ba inisializasaun sistema...')),
      );
      return;
    }
            ...
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasError) {
                  return const Center(child: Text('Iha erru ida'));
                }

...
class _ManageContentScreenState extends State<ManageContentScreen> {
  ...
  Future<void> _addContent() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Konteúdu aumenta ho susesu')),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('La konsege aumenta konteúdu: $e')),
      );
    } finally {
      setState(() {
        _isLoading = false;
  ...
  bool _validateForm() {
    if (_titleController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Títulu tenke prenxe iha')),
      );
      return false;
    }
    ...
    if (_categoryController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Kategoria tenke hili')),
      );
      return false;
    }
    ...
    if (_thumbnailBase64 == null &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Imajen ki ik sira tenke hili')),
      );
      return false;
    }
    ...
    if (_currentContentType == ContentType.article) {
      if (_descriptionController.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Deskrisaun artigu nian tenke prenxe iha')),
        );
        return false;
      }
      if (_readTimeController.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Tempu lee nian tenke prenxe')),
        );
        return false;
      }
    } else {
      if (_youtubeUrlController.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('URL YouTube nian maka presiza')),
        );
        return false;
      }
      if (!_isValidYouTubeUrl(_youtubeUrlController.text.trim())) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('URL YouTube ne ebé la válidu')),
        );
        return false;
      }
    }
      ...
      child: Column(
        children: [
          TabBar(
            ...
            tabs: const [Tab(text: 'Artigu'), Tab(text: 'Vídeo sira')],
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Center(child: Text('Laiha artigu sira'));
        }
  ...
  Widget _buildVideosList() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Center(child: Text('Laiha vídeo sira'));
        }
        return ListView.builder(
          itemCount: snapshot.data!.docs.length,
          itemBuilder: (context, index) {
            ...
            child: Column(
              ...
              children: [
                ...
                if (type == ContentType.article)
                  Text(
                    '${contentData['readTime']} minutu lee',
                    style: TextStyle(color: Colors.grey[600], fontSize: 12),
                  ),
  ...
  Future<void> _deleteContent(String contentId, ContentType type) async {
    final bool? confirmDelete = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Hamoos Konteúdu'),
        content: Text(
            'Ita-boot iha serteza katak ita-boot hakarak atu hamoos ${type == ContentType.article ? 'artikel' : 'video'} Ida ne e?'),
        actions: [
          TextButton(
    ...
    if (confirmDelete == true) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                '${type == ContentType.article ? 'Artikel' : 'Video'} hamoos ho susesu'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('La konsege hamoos konteúdu: $e'),
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
              labelText:
                  'Titulu ${_currentContentType == ContentType.article ? 'Artikel' : 'Video'}',
          ...
          if (_currentContentType == ContentType.article) ...[
            TextFormField(
              ...
              maxLines: 4,
              decoration: InputDecoration(
                labelText: 'Deskrisaun Artigu nian',
                border: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(10),
                ),
            ...
            TextFormField(
              ...
              decoration: InputDecoration(
                labelText: 'Tempu Leitura (minutus)',
                border: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(10),
                ),
            ...
            TextFormField(
              ...
              decoration: InputDecoration(
                labelText: 'URL sira YouTube nian',
          ...
          ElevatedButton(
            ...
            child: _isLoading
                ...
                : Text(
                    'Plus ${_currentContentType == ContentType.article ? 'Artikel' : 'Video'}',
                    style: const TextStyle(
                      fontSize: 16,
